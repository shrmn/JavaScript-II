// A local community center is holding a fund rasising 5k fun run and has invited 50 small businesses to make a small donation 
// on their behalf for some much needed updates to their facilities.  Each business has assigned a representative to attend the event 
...
let allCaps = [];
const drunkDirector = runners.map(function(currentValue) {
  let capitalized = currentValue["first_name"].toUpperCase();
  allCaps.push(capitalized);
})
console.log(allCaps); 
